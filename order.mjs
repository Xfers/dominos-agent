...
if (constraints.isDelivery) {
  ...
  try {
    ...
    // Wait longer for menu page to load after scheduling
    for (let w = 0; w < 10; w++) {
      await page.waitForTimeout(1500);
      if (page.url().includes('/menu')) { storeOk = true; break; }
    }
    if (storeOk) console.log('  Delivery: ✓');
    else {
      // One more attempt: check if we're on some intermediary page and click through
      await page.screenshot({ path: `${SS}/delivery-debug.png` });
      const pageState = await page.evaluate(() => ({
        url: location.href,
        text: document.body.innerText.substring(0, 200),
        buttons: [...document.querySelectorAll('button, [role="button"]')].filter(el => el.getBoundingClientRect().height > 0).map(el => el.textContent.trim().substring(0, 30)).slice(0, 10)
      }));
      console.log('  Delivery stuck:', JSON.stringify(pageState));
    }
  } catch(e) { console.log('  Delivery error:', e.message?.substring(0,60)); }
}

if (constraints.isDelivery && !storeOk) {
  console.log('  ✗ Delivery failed - user explicitly requested delivery, NOT falling back to pickup');
  console.log('  Retrying delivery from scratch...');
  try {
    await page.goto('https://www.dominos.com.tw/', { waitUntil: 'domcontentloaded', timeout: 30000 });
    await page.waitForTimeout(4000);
    await page.getByText('外送', { exact: true }).first().click();
    await page.waitForTimeout(2000);
    const addrInput2 = page.locator('[data-testid="delivery-address-search.search-input"] input, input[placeholder*="輸入"]').first();
    await addrInput2.click();
    await page.waitForTimeout(500);
    let addr2 = USER_ADDRESS;
    let searchAddr2 = addr2.replace(/^\d{3,5}/, '').replace(/^台灣/, '').replace(/^臺北市|^台北市/, '').replace(/^.{2,3}區/, '').replace(/\d+樓.*$/, '').trim();
    if (!searchAddr2 || searchAddr2.length < 3) searchAddr2 = addr2;
    console.log(`    Retry address: ${searchAddr2}`);
    await addrInput2.fill(searchAddr2);
    await page.waitForTimeout(3000);
    const predOption2 = page.locator('[data-testid="delivery-address-predictions-results.options.option"]').first();
    if (await predOption2.count() > 0) {
      await predOption2.click();
      await page.waitForTimeout(3000);
    }
    const confirmBtn2 = page.locator('[data-testid="confirm-address.button"]');
    for (let w = 0; w < 8; w++) {
      if (await confirmBtn2.count() > 0) break;
      await page.waitForTimeout(1000);
    }
    if (await confirmBtn2.count() > 0) {
      const floorMatch2 = addr2.match(/(\d+)樓/);
      if (floorMatch2) {
        const floorInput2 = page.locator('[data-testid="address-field-floorNumber"] input').first();
        if (await floorInput2.count() > 0) await floorInput2.fill(floorMatch2[1]);
      }
      await confirmBtn2.click();
      await page.waitForTimeout(4000);
    }
    // For retry, always use order-later with scheduling
    const expander2 = page.locator('[data-testid="order-later-expander-button"]');
    for (let w = 0; w < 8; w++) {
      if (await expander2.count() > 0) break;
      const hasLater = await page.locator('[data-testid="start-order-later-button"]').count();
      if (hasLater > 0) break;
      await page.waitForTimeout(1500);
    }
    if (await expander2.count() > 0) {
      await expander2.click({ force: true });
      await page.waitForTimeout(2000);
    }
    const laterBtn2 = page.locator('[data-testid="start-order-later-button"]');
    if (await laterBtn2.count() > 0) {
      await laterBtn2.click({ force: true });
      await page.waitForTimeout(8000);
      if (page.url().includes('/menu')) { storeOk = true; console.log('  Delivery retry: ✓'); }
    }
  } catch(e2) { console.log('  Delivery retry error:', e2.message?.substring(0,60)); }
}

if (!storeOk && constraints.isDelivery) {
  console.log('  ✗ Delivery failed after retry. Cannot proceed with pickup when delivery was requested.');
  console.log('  The store may not support delivery scheduling at this time.');
  await browser.close();
  process.exit(1);
}

if (!storeOk) {
  // Pickup fallback (only when delivery was NOT requested)
  await page.goto('https://www.dominos.com.tw/', { waitUntil: 'domcontentloaded', timeout: 30000 });
  await page.waitForTimeout(4000);
...
if (!promoUsed) {
  ...
  await page.goto('https://order.dominos.com.tw/menu/pizza', { waitUntil: 'domcontentloaded', timeout: 20000 });
  ...
  if (exactOrder && (exactOrder.wantCoupon || hasMultipleLargePizzas)) {
    ...
    // Wait for best voucher (送可樂) to load; fallback after timeout
    let bestVoucherReady = false;
    for (let w = 0; w < 20; w++) {
      const check = await page.evaluate(() => {
        const text = document.body.innerText;
        if (text.includes('送可樂')) return 'best';
        if (document.querySelectorAll('[data-testid*="lovable-menu"]').length > 3 || text.includes('買大送')) return 'partial';
        return 'none';
      });
      if (check === 'best') { bestVoucherReady = true; break; }
      if (check === 'partial' && w >= 10) break;
      await page.waitForTimeout(500);
    }
    ...
    if (voucherApplied.found) {
      ...
      promoUsed = true;
      await page.waitForTimeout(1500);
      // Re-navigate to pizza menu to reset page state after voucher activation
      await page.goto('https://order.dominos.com.tw/menu/pizza', { waitUntil: 'domcontentloaded', timeout: 20000 });
      for (let w = 0; w < 8; w++) {
        const hasMenu = await page.evaluate(() => document.querySelectorAll('[data-testid*="lovable-menu"]').length > 5);
        if (hasMenu) break;
        await page.waitForTimeout(500);
      }
    } else {
...
// Try to expand contact section if collapsed (delivery orders have it hidden)
let needsFill = await page.evaluate(() => !!document.querySelector('input[aria-label="姓名*"]'));
if (!needsFill) {
  // Click on the contact/details section to expand it
  const expanded = await page.evaluate(() => {
    // Look for section headers or edit buttons related to contact info
    const targets = [...document.querySelectorAll('button, [role="button"], div, span, a')].filter(el => {
      const t = el.textContent.trim();
      const testId = el.getAttribute('data-testid') || '';
      return (t.includes('聯絡') || t.includes('個人') || t.includes('詳細') || t.includes('資料') ||
              testId.includes('details') || testId.includes('contact') || testId.includes('my-details') ||
              (t.includes('編輯') && el.closest('[data-testid*="detail"]'))) &&
             t.length < 30 && el.getBoundingClientRect().height > 0;
    });
    if (targets.length > 0) { targets[0].click(); return targets[0].textContent.trim(); }
    // Also try data-testid patterns for Domino's
    const editBtn = document.querySelector('[data-testid="my-details.edit-button"], [data-testid="checkout-details.edit"], [data-testid*="details"][data-testid*="edit"]');
    if (editBtn) { editBtn.click(); return 'edit-btn:' + (editBtn.getAttribute('data-testid') || ''); }
    return null;
  });
  if (expanded) console.log(`  Expanded contact section: ${expanded}`);
  await page.waitForTimeout(2000);
  needsFill = await page.evaluate(() => !!document.querySelector('input[aria-label="姓名*"]'));
}
if (!needsFill) {
  // Last try: look for any visible input fields related to contact
  const formState = await page.evaluate(() => {
    const inputs = [...document.querySelectorAll('input')].filter(el => el.getBoundingClientRect().height > 0);
    return inputs.map(el => ({ aria: el.getAttribute('aria-label'), placeholder: el.placeholder, name: el.name, testId: el.getAttribute('data-testid') })).slice(0, 10);
  });
  console.log('  Contact form not found. Visible inputs:', JSON.stringify(formState));
}
if (needsFill) {
...
// Check for gateway error (only if no card form present)
const gatewayError = await page.evaluate(() => {
  const hasCardForm = document.querySelector('input[name="cardNo2"], input[name*="card"], input[maxlength="16"]');
  if (hasCardForm) return null;
  const text = document.body.innerText;
  if (text.includes('merchantId Incorrect') || (text.includes('Error Message') && text.includes('Parameter'))) {
    const m = text.match(/Error Message[:\s]*(.*)/i);
    return m ? m[1].trim() : text.substring(0, 100);
  }
  return null;
});
if (gatewayError) {
...
console.log(`  Card filled in ${Date.now() - fillStart}ms (total elapsed: ${((Date.now() - DEMO_START)/1000).toFixed(1)}s)`);
...
if (SUBMIT_PAYMENT) {
  ...
  console.log('  Test mode: waiting 5s then closing...');
  await page.waitForTimeout(5000);
  await browser.close();
}
